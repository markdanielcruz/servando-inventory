    ...
    if not log_df.empty:
        ...
        if del_log.empty:
            ...
            if sel_ref:
                ...
                ITEMS_PER_PAGE = 25
                all_rows = list(ref_rows.iterrows())
                total_pages = max(1, -(-len(all_rows) // ITEMS_PER_PAGE))
                pages_html = ""
                for pg in range(total_pages):
                    chunk = all_rows[pg*ITEMS_PER_PAGE:(pg+1)*ITEMS_PER_PAGE]
                    is_last = (pg == total_pages - 1)
                    continued_note = f'<p class="doc-continued">Page {{pg+1}} of {{total_pages}} — continued</p>' if pg > 0 else ""
                    chunk_rows = ""
                    for ri, (_, r) in enumerate(chunk, pg*ITEMS_PER_PAGE+1):
                        qty = num(r["ADD_IN"])
                        unit_cost = num(items_df[items_df["ITEM"]==r["ITEM"]]["UNIT COST"].values[0]) if r["ITEM"] in items_df["ITEM"].values else 0
                        uom = items_df[items_df["ITEM"]==r["ITEM"]]["UNIT OF MEASURE"].values[0] if r["ITEM"] in items_df["ITEM"].values else ""
                        val = qty * unit_cost
                        qty_fmt = f"{int(qty):,}" if qty == int(qty) else f"{qty:,.2f}"
                        chunk_rows += f"<tr><td>{{ri}}</td><td class=\"item-name\">{{r[\"ITEM\"]}}</td><td class=\"r\">{{qty_fmt}}</td><td>{{uom}}</td><td class=\"r\">&#8369;{{val:,.2f}}</td></tr>"
                    total_section = ""
                    if is_last:
                        total_section = f"""<tr class="tfoot-row"><td colspan="4" class="r">Total</td><td class="r">&#8369;{total_val:,.2f}</td></tr>"""
                        sig_section = """<div class="signatures">
                          <div class="sig"><div class="sig-line"></div><div class="sig-label">Received by</div></div>
                          <div class="sig"><div class="sig-line"></div><div class="sig-label">Checked by</div></div>
                          <div class="sig"><div class="sig-line"></div><div class="sig-label">Noted by</div></div>
                        </div>"""
                    else:
                        sig_section = ""
                    pages_html += f"""<div class="page">
                      <div class="doc-header">
                        <div class="doc-header-left">
                          <p class="doc-type">Incoming Delivery</p>
                          <p class="doc-ref">{sel_ref}</p>
                          {{continued_note}}
                        </div>
                        <img class="logo" src="data:image/jpeg;base64,{{LOGO_B64}}" alt="Servando"/>
                      </div>
                      <div class="meta">
                        <div class="meta-item"><label>Date</label><span>{first.get("DATE","")}</span></div>
                        <div class="meta-item"><label>Received by</label><span>{first.get("STAFF","")}</span></div>
                        <div class="meta-item"><label>Items</label><span>{{len(ref_rows)}}</span></div>
                      </div>
                      <table>
                        <colgroup><col class="c-no"><col class="c-item"><col class="c-qty"><col class="c-unit"><col class="c-amt"></colgroup>
                        <thead><tr><th>#</th><th>Item</th><th class="r">Qty</th><th>Unit</th><th class="r">Amount</th></tr></thead>
                        <tbody>{{chunk_rows}}{{total_section}}</tbody>
                      </table>
                      {{sig_section}}
                      <div class="footer">Servando &nbsp;·&nbsp; Main Warehouse &nbsp;·&nbsp; {sel_ref} &nbsp;·&nbsp; Page {{pg+1}} of {{total_pages}}</div>
                    </div>"""
                print_html = """<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
  *{{margin:0;padding:0;box-sizing:border-box;}}
  body{{font-family:Arial,sans-serif;font-size:9px;color:#111;background:#fff;padding:20px 28px;}}
  .page{{page-break-after:always;margin-bottom:40px;}}
  .page:last-child{{page-break-after:avoid;margin-bottom:0;}}
  .doc-header{{display:flex;justify-content:space-between;align-items:center;border-bottom:2px solid #111;padding-bottom:10px;margin-bottom:12px;}}
  .doc-type{{font-size:14px;font-weight:700;color:#111;margin:0 0 2px;}}
  .doc-ref{{font-size:8px;color:#888;margin:0;}}
  .doc-continued{{font-size:8px;color:#888;font-style:italic;margin:2px 0 0;}}
  .logo{{height:36px;width:auto;object-fit:contain;}}
  .meta{{display:flex;gap:0;margin-bottom:12px;border:1px solid #ddd;}}
  .meta-item{{flex:1;padding:6px 10px;border-right:1px solid #ddd;}}
  .meta-item:last-child{{border-right:none;}}
  .meta-item label{{font-size:7px;letter-spacing:1.5px;text-transform:uppercase;color:#888;display:block;margin-bottom:3px;}}
  .meta-item span{{font-size:10px;font-weight:600;color:#111;}}
  table{{width:100%;border-collapse:collapse;table-layout:fixed;}}
  col.c-no{{width:5%;}} col.c-item{{width:52%;}} col.c-qty{{width:13%;}} col.c-unit{{width:13%;}} col.c-amt{{width:17%;}}
  thead tr{{border-top:1.5px solid #111;border-bottom:1.5px solid #111;}}
  th{{padding:5px 4px;text-align:left;font-size:7px;letter-spacing:1.2px;text-transform:uppercase;font-weight:700;}}
  th.r,td.r{{text-align:right;}}
  tbody tr{{border-bottom:1px solid #eee;}}
  td{{padding:4px 4px;font-size:10px;color:#111;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}}
  td.item-name{{white-space:normal;word-break:break-word;line-height:1.3;}}
  .tfoot-row td{{border-top:1.5px solid #111;padding:6px 4px;font-size:10px;font-weight:700;}}
  .signatures{{display:flex;gap:20px;margin-top:32px;}}
  .sig{{flex:1;}}
  .sig-line{{border-top:1px solid #111;margin-bottom:5px;}}
  .sig-label{{font-size:7px;letter-spacing:1.2px;text-transform:uppercase;color:#888;}}
  .footer{{margin-top:16px;border-top:1px solid #ddd;padding-top:6px;text-align:center;font-size:7px;letter-spacing:1.5px;text-transform:uppercase;color:#bbb;}}
  @media print{{.no-print{{display:none;}} body{{padding:12px 18px;}}}}
</style></head><body>""" + pages_html + """<br/><button class="no-print" onclick="window.print()" style="margin-top:16px;padding:8px 20px;font-size:11px;font-weight:700;letter-spacing:1px;text-transform:uppercase;background:#111;color:#fff;border:none;cursor:pointer;">&#128438; Print</button></body></html>"""

                import base64
                b64 = base64.b64encode(print_html.encode()).decode()
                st.markdown(f'<a href="data:text/html;base64,{b64}" target="_blank"><button style="background:#2A3E28;color:#A8C896;border:1px solid #3A5238;border-radius:8px;padding:8px 18px;font-weight:600;cursor:pointer;margin-top:8px;">🖨️ Open Print View</button></a>', unsafe_allow_html=True)
    else:
        st.info("No past deliveries yet.")
            ...
            if sel_po_ref:
                ...
                for pg in range(total_pages):
                    ...
                    pages_html += f"""<div class="page">
                      <div class="doc-header">
                        <div class="doc-header-left">
                          <p class="doc-type">Purchase Order</p>
                          <p class="doc-ref">{sel_po_ref}</p>
                          {{continued_note}}
                        </div>
                        <img class="logo" src="data:image/jpeg;base64,{{LOGO_B64}}" alt="Servando"/>
                      </div>
                      <div class="meta">
                        <div class="meta-item"><label>Date</label><span>{first.get("DATE","")}</span></div>
                        <div class="meta-item"><label>Prepared by</label><span>{first.get("STAFF","")}</span></div>
                        <div class="meta-item"><label>Department</label><span>{{dept_display}}</span></div>
                        <div class="meta-item"><label>Items</label><span>{{len(ref_rows)}}</span></div>
                      </div>
                      <table>
                        <colgroup><col class="c-no"><col class="c-item"><col class="c-qty"><col class="c-unit"><col class="c-amt"></colgroup>
                      ...
                      {{sig_section}}
                      <div class="footer">Servando &nbsp;·&nbsp; Main Warehouse &nbsp;·&nbsp; {sel_po_ref} &nbsp;·&nbsp; Page {{pg+1}} of {{total_pages}}</div>
                    </div>"""
                print_html = """<!DOCTYPE html><html><head><meta charset="UTF-8"><style>
  *{{margin:0;padding:0;box-sizing:border-box;}}
  body{{font-family:Arial,sans-serif;font-size:9px;color:#111;background:#fff;padding:20px 28px;}}
