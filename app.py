    ...
    if not log_df.empty:
        ...
        if del_log.empty:
            ...
            if sel_ref:
                ...
                import base64 as _b64, tempfile as _tmp, os as _os, struct as _struct, zlib as _zlib
                ...
                ws["A1"] = "INCOMING DELIVERY"
                ws["A1"].font = title_font
                ws["A1"].alignment = Alignment(vertical="center")
                ws.row_dimensions[1].height = 22

                ws.merge_cells("A2:E2")
                ws["A2"] = f"Reference: {sel_ref}"
                ws["A2"].font = sub_font
                ...
                for col, label, val in [
                    ("A", "Date", first.get("DATE","")),
                    ("C", "Received By", first.get("STAFF","")),
                    ("E", "Items", str(len(ref_rows))),
                ]:
                    ws[f"{col}4"] = label
                    ws[f"{col}4"].font = Font(bold=True, name="Arial", size=8, color="888888")
                    ws[f"{col}5"] = val
                    ws[f"{col}5"].font = Font(bold=True, name="Arial", size=11)
                ws.row_dimensions[4].height = 12
                ws.row_dimensions[5].height = 16
                ws.row_dimensions[6].height = 8

                # ── table header ──
                for ci, hdr in enumerate(["#","Item","Unit","Qty","Amount"], 1):
                ...
                for ri, (_, r) in enumerate(ref_rows.iterrows(), 1):
                    ...
                    val = qty * unit_cost
                    total_val += val
                    qty_fmt = int(qty) if qty == int(qty) else round(qty, 2)
                    row_num = 7 + ri
                    fill = grey_fill if ri % 2 == 0 else PatternFill()
                    for ci, v in enumerate([ri, r["ITEM"], uom, qty_fmt, val], 1):
                ...
                # ── total row ──
                total_row = 7 + len(ref_rows) + 1
                ws.merge_cells(f"A{total_row}:D{total_row}")
                ws[f"A{total_row}"] = "TOTAL"
                ws[f"A{total_row}"].font = total_font
                ws[f"A{total_row}"].alignment = Alignment(horizontal="right")
                ws[f"A{total_row}"].border = bdr
                ws[f"E{total_row}"] = total_val
                ws[f"E{total_row}"].font = total_font
                ws[f"E{total_row}"].number_format = "₱#,##0.00"
                ws[f"E{total_row}"].alignment = Alignment(horizontal="right")
                ws[f"E{total_row}"].border = bdr
                ws[f"E{total_row}"].fill = PatternFill("solid", fgColor="E8F0E8")
                ws.row_dimensions[total_row].height = 18

                # ── signature rows ──
                sig_row = total_row + 3
                ws.row_dimensions[sig_row-1].height = 24
                for ci, label in [(1,"Received By"),(3,"Checked By"),(5,"Noted By")]:
                    ws.cell(row=sig_row-1, column=ci).border = Border(bottom=Side(style="thin", color="111111"))
                    ws.cell(row=sig_row, column=ci, value=label).font = Font(name="Arial", size=8, color="888888")

                # ── footer ──
                footer_row = sig_row + 2
                ws.merge_cells(f"A{footer_row}:E{footer_row}")
                ws[f"A{footer_row}"] = f"Servando  ·  Main Warehouse  ·  {sel_ref}"
                ws[f"A{footer_row}"].font = Font(name="Arial", size=7, color="BBBBBB")
    ...
    if not log_df.empty:
        ...
        if po_log.empty:
            ...
            if sel_po_ref:
                ...
                for col, label, val in [
                    ("A", "Date", first.get("DATE","")),
                    ("B", "Prepared By", first.get("STAFF","")),
                    ("C", "Department", dept_display),
                    ("E", "Items", str(len(ref_rows))),
                ]:
                    ws[f"{col}4"] = label
                    ws[f"{col}4"].font = Font(bold=True, name="Arial", size=8, color="888888")
                ...
                sig_row = total_row + 3
                ws.row_dimensions[sig_row-1].height = 24
                for ci, label in [(1,"Prepared By"),(3,"Approved By"),(5,"Received By")]:
                    ws.cell(row=sig_row-1, column=ci).border = Border(bottom=Side(style="thin", color="111111"))
                    ws.cell(row=sig_row, column=ci, value=label).font = Font(name="Arial", size=8, color="888888")

                footer_row = sig_row + 2
                ws.merge_cells(f"A{footer_row}:E{footer_row}")
                ws[f"A{footer_row}"] = f"Servando  ·  Main Warehouse  ·  {sel_po_ref}"
                ws[f"A{footer_row}"].font = Font(name="Arial", size=7, color="BBBBBB")
                ws[f"A{footer_row}"].alignment = Alignment(horizontal="center")
